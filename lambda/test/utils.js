...
describe('utils', () => {
    describe('#secondsToTime()', () => {
        it('should work for 00:00', () => {
            const time = utils.secondsToTime(0);
            expect(time).to.equal('00:00');
        });

        it('should work for 00:01', () => {
            const time = utils.secondsToTime(1);
            expect(time).to.equal('00:01');
        });

        it('should work for 00:32', () => {
            const time = utils.secondsToTime(32);
            expect(time).to.equal('00:32');
        });

        it('should work for 01:00', () => {
            const time = utils.secondsToTime(60);
            expect(time).to.equal('01:00');
        });

        it('should work for 01:01', () => {
            const time = utils.secondsToTime(61);
            expect(time).to.equal('01:01');
        });

        it('should work for 01:32', () => {
            const time = utils.secondsToTime(92);
            expect(time).to.equal('01:32');
        });

        it('should work for 01:00:00', () => {
            const time = utils.secondsToTime(3600);
            expect(time).to.equal('01:00:00');
        });

        it('should work for 10:00:00', () => {
            const time = utils.secondsToTime(36000);
            expect(time).to.equal('10:00:00');
        });
    });
    ...
    describe('#fixSong()', () => {
        ...
        it('should work for Hans-Erik Phillip', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: 'Hans-Erik Phillip',
                    title: 'Fiskerne',
                    album: 'Original Soundtrack from Fiskerne',
                };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for Corazon Espinado (feat Mana)', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: 'Santana',
                    title: 'Corazon Espinado (feat Mana)',
                    album: 'Supernatural',
                };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for Face à la mer (Massive Attack remix)', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: 'Les Negresses Vertes',
                    title: 'Face a la mer (Massive Attack remix)',
                    album: '10 Remixes 87-93',
                };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for Fior Di Nha Esperanca', () => {
            locales.forEach((locale) => {
                const song = { ...EMPTY_SONG, artist: 'Cesária Évora', title: 'Fior Di Nha Esperanca', album: '' };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for Gnossienne No1', () => {
            locales.forEach((locale) => {
                const song = { ...EMPTY_SONG, artist: 'Erik Satie', title: 'Gnossienne No1', album: '' };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for Morning Mood (Allegretto pas', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: 'Edvard Grieg',
                    title: 'Morning Mood (Allegretto pas',
        ...
        it('should work for Braided Hair (w/ Neneh Cherry & Speech)', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: '1 Giant Leap',
                    title: 'Braided Hair (w/ Neneh Cherry & Speech)',
                    album: '1 Giant Leap',
                };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for My Culture (w/Robbie Williams & Maxi Jazz)', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: '1 Giant Leap',
                    title: 'My Culture (w/Robbie Williams & Maxi Jazz)',
                    album: '1 Giant Leap',
                };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for Silent All These Years (Live w/ Leonard Cohen)', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: 'Tori Amos',
                    title: 'Silent All These Years (Live w/ Leonard Cohen)',
        ...
        it('should work for 20 Jahre: Nena Ft Nena', () => {
            locales.forEach((locale) => {
                const song = {
                    ...EMPTY_SONG,
                    artist: 'Nena',
                    title: 'Anyplace, Anywhere, Anytime (w/ Kim Wilde)',
                    album: '20 Jahre: Nena Ft Nena',
                };
                const fixedSong = utils.fixSong(song, locale);
        ...
        it('should work for Real Live Roadrunning (w/ Mark Knopfler)', () => {
            locales.forEach((locale) => {
                const song = {
                    ...
                    title: 'Red Dirt Girl (w/ Mark Knopfler)',
        ...
        it('should work for Yellow/Gold', () => {
            locales.forEach((locale) => {
                const song = {
                    ...
                    title: 'Watch the Moon Disappear',
                    album: 'Yellow/Gold',
                };
                const fixedSong = utils.fixSong(song, locale);
    ...
    describe('#speakArtist()', () => {
        ...
        it('should work for 2raumwohnung', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ? '2raumwohnung'
                    : '<lang xml:lang="de-DE">2raumwohnung</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Alexandra Stréliski', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Alexandra Stréliski'
                    : '<lang xml:lang="fr-FR">Alexandra Stréliski</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Antonio Vivaldi', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('it')
                    ? 'Antonio Vivaldi'
                    : '<lang xml:lang="it-IT">Antonio Vivaldi</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Carlos Núñez', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ? 'Carlos Núñez'
                    : '<lang xml:lang="es-ES">Carlos Núñez</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Chicha Libre', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ? 'Chicha Libre'
                    : '<lang xml:lang="es-ES">Chicha Libre</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Claude Debussy', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Claude Debussy'
                    : '<lang xml:lang="fr-FR">Claude Debussy</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for José González', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ? 'José González'
                    : '<lang xml:lang="es-ES">José González</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Kruder & Dorfmeister', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ? 'Kruder &amp; Dorfmeister'
                    : '<lang xml:lang="de-DE">Kruder &amp; Dorfmeister</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Maurice Ravel', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Maurice Ravel'
                    : '<lang xml:lang="fr-FR">Maurice Ravel</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Paco de Lucía', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ? 'Paco de Lucía'
                    : '<lang xml:lang="es-ES">Paco de Lucía</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Rubén González', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ? 'Rubén González'
                    : '<lang xml:lang="es-ES">Rubén González</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Peter Schilling', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ? 'Peter Schilling'
                    : '<lang xml:lang="de-DE">Peter Schilling</lang>';
                expect(artist, locale).to.equal(expected);
            });
        });
    ...
    describe('#speakTitle()', () => {
        it('should work for Blue In Green', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('en')
                    ? 'Blue In Green'
                    : '<lang xml:lang="en-US">Blue In Green</lang>';
        ...
        it('should work for 2 von Millionen von Sternen', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ...
                    : '<lang xml:lang="de-DE">2 von Millionen von Sternen</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it("should work for 200 Ans d'Hypocrosie", () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? '200 Ans d&#39;Hypocrosie'
                    : '<lang xml:lang="fr-FR">200 Ans d&#39;Hypocrosie</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for A Pesar De Todo', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ? 'A Pesar De Todo'
                    : '<lang xml:lang="es-ES">A Pesar De Todo</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Alma, Corazon Y Vida', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ? 'Alma, Corazon Y Vida'
                    : '<lang xml:lang="es-ES">Alma, Corazon Y Vida</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Brandenburg Concerto No. 5 In D', () => {
            const song = {
                ...
                title: 'Brandenburg Concerto No. 5 In D',
        ...
        it("should work for C'est La Vie (w/ Natacha Atlas)", () => {
            locales.forEach((locale) => {
                ...
                let expected =
                    '<lang xml:lang="fr-FR">C\'est La Vie</lang><lang xml:lang="en-US"> (w/ Natacha Atlas)</lang>';
                if (locale.startsWith('fr')) {
        ...
        it('should work for Camions Sauvages', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Camions Sauvages'
                    : '<lang xml:lang="fr-FR">Camions Sauvages</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Canción Del Mariachi', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ...
                    : '<lang xml:lang="es-ES">Canción Del Mariachi</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it("should work for Ce N'est Pas Bon", () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Ce N&#39;est Pas Bon'
                    : '<lang xml:lang="fr-FR">Ce N&#39;est Pas Bon</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Danza di Cala Luna (w/ John Williams & Paco Pena)', () => {
            locales.forEach((locale) => {
                ...
                let expected =
                    '<lang xml:lang="it-IT">Danza di Cala Luna</lang><lang xml:lang="en-US"> (with John Williams & Paco Pena)</lang>';
        ...
        it('should work for Eine Kleine Nachtmusik - Allegro', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ...
                    : '<lang xml:lang="de-DE">Eine Kleine Nachtmusik - Allegro</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Face à la mer (Massive Attack remix)', () => {
            locales.forEach((locale) => {
                ...
                let expected =
                    '<lang xml:lang="fr-FR">Face à la mer</lang><lang xml:lang="en-US"> (Massive Attack remix)</lang>';
                if (locale.startsWith('fr')) {
        ...
        it('should work for Fly Like A Bird (Acoustic Live)', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('en')
                    ? 'Fly Like A Bird (Acoustic <w role="amazon:NN">Live</w>)'
                    : '<lang xml:lang="en-US">Fly Like A Bird (Acoustic <w role="amazon:NN">Live</w>)</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Gnossienne No. 1', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Gnossienne No. 1'
                    : '<lang xml:lang="fr-FR">Gnossienne No. 1</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Gymnopedie No. 1', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Gymnopedie No. 1'
                    : '<lang xml:lang="fr-FR">Gymnopedie No. 1</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Ich Weiß Warum', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ? 'Ich Weiß Warum'
                    : '<lang xml:lang="de-DE">Ich Weiß Warum</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Jardins sous la pluie', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ...
                    : '<lang xml:lang="fr-FR">Jardins sous la pluie</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it("should work for L'Amour est blue (Love is blue)", () => {
            locales.forEach((locale) => {
                const title = utils.speakTitle("L'Amour est blue (Love is blue)", locale);
                let expected =
                    '<lang xml:lang="fr-FR">L&#39;Amour est blue</lang><lang xml:lang="en-US"> (Love is blue)</lang>';
                if (locale.startsWith('fr')) {
        ...
        it('should work for La Femme Accident', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'La Femme Accident'
                    : '<lang xml:lang="fr-FR">La Femme Accident</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it("should work for La Femme D'Argent", () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'La Femme D&#39;Argent'
                    : '<lang xml:lang="fr-FR">La Femme D&#39;Argent</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Le Voyage De Penelope', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ...
                    : '<lang xml:lang="fr-FR">Le Voyage De Penelope</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Mer Du Japon', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Mer Du Japon'
                    : '<lang xml:lang="fr-FR">Mer Du Japon</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for On the Border (live)', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('en')
                    ? 'On the Border (<w role="amazon:NN">live</w>)'
                    : '<lang xml:lang="en-US">On the Border (<w role="amazon:NN">live</w>)</lang>';
                expect(title, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Overture (Concierto de Aranjuez)', () => {
            locales.forEach((locale) => {
                ...
                let expected =
                    '<lang xml:lang="en-US">Overture</lang><lang xml:lang="es-ES"> (Concierto de Aranjuez)</lang>';
        ...
        it('should work for Vivaldi - Allegro, Concerto in G Major, Op 4 No 3', () => {
            const song = {
                ...
                title: 'Vivaldi - Allegro, Concerto in G Major, Op 4 No 3',
    ...
    describe('#speakAlbum()', () => {
        it('should work for Kind Of Blue', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('en')
                    ? 'Kind Of Blue'
                    : '<lang xml:lang="en-US">Kind Of Blue</lang>';
        ...
        it('should work for 20 Jahre: Nena feat. Nena', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ...
                    : '<lang xml:lang="de-DE">20 Jahre: Nena feat. Nena</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
        ...
        it('should work for 99 Luftballons', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ? '99 Luftballons'
                    : '<lang xml:lang="de-DE">99 Luftballons</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Alles Prima Und Viele Andere Hits', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ? 'Alles Prima Und Viele Andere Hits'
                    : '<lang xml:lang="de-DE">Alles Prima Und Viele Andere Hits</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Concierto de Aranjuez (Joaquin Rodrigo, 1939)', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('es')
                    ...
                    : '<lang xml:lang="es-ES">Concierto de Aranjuez (Joaquin Rodrigo, 1939)</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Des Visages des Figures', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ...
                    : '<lang xml:lang="fr-FR">Des Visages des Figures</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Eine Kleine Nachtmusik', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ...
                    : '<lang xml:lang="de-DE">Eine Kleine Nachtmusik</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Hélène Grimaud - Bach', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('fr')
                    ? 'Hélène Grimaud - Bach'
                    : '<lang xml:lang="fr-FR">Hélène Grimaud - Bach</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
        ...
        it('should work for Kommt zusammen', () => {
            locales.forEach((locale) => {
                ...
                const expected = locale.startsWith('de')
                    ? 'Kommt zusammen'
                    : '<lang xml:lang="de-DE">Kommt zusammen</lang>';
                expect(album, locale).to.equal(expected);
            });
        });
